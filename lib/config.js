...
const importDynamic_1 = require("./importDynamic");
        ...
        for (const p of this.plugins) {
            ...
            for (const hook of hooks) {
                try {
                    ...
                    const f = p.module ? require.resolve(path.join(p.root, hook)) : ts_node_1.tsPath(p.root, hook);
                    debug('start', p.module ? '(import)' : '(require)', f);
                    await search(p.module ? await importDynamic_1.default(f) : require(f)).call(context, Object.assign(Object.assign({}, opts), { config: this }));
                    debug('done');
                }
                catch (error) {
